...
export class VSCodeNotebookController implements Disposable, IVSCodeNotebookController {
    ...
    public async connectToKernel(
        ...
        return KernelConnector.connectToNotebookKernel(
            ...
            { resource, notebook, controller: this.controller },
            options,
            this.disposables,
            'jupyterExtension',
            onAction,
            async (action: KernelAction, actionSource: KernelActionSource, kernel: IKernel) => {
                await this.onKernelActionCompleted(action, actionSource, kernel as IKernel);
                await onActionCompleted(action, actionSource, kernel as IKernel);
            }
        );
    }
    ...
    private async executeCell(notebook: NotebookDocument, cell: NotebookCell) {
        ...
        try {
            ...
            await endCellAndDisplayErrorsInCell(
                cell,
                controller,
                await errorHandler.getErrorMessageForDisplayInCell(ex, currentContext, notebook.uri),
                isCancelled
            );
        }
    ...
    private async onDidSelectController(notebook: NotebookDocument) {
        ...
        if (this.kernelProvider.get(notebook)?.controller.id === this.id) {
            // Possible we've controller to the live version.
            // Assume you have a remote kernel, when we start the kernel, we will
            // swap the kernel from the remote kernelspec to the live kernel.
            return;
        }
        const newKernel = this.kernelProvider.getOrCreate(notebook, {
